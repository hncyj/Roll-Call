...
class StatusPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self._called_label = QLabel("本轮已点名（0）")
        ...
        self._uncalled_label = QLabel("本轮未点名（0）")
    ...
    def refresh(self, called: List[Student], uncalled: List[Student]) -> None:
        self._called_label.setText(f"本轮已点名（{len(called)}）")
        self._called_list.clear()
        for s in called:
            self._called_list.addItem(f"{s.id}  {s.name}")

        self._uncalled_label.setText(f"本轮未点名（{len(uncalled)}）")
